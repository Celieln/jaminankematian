...
<title>Edit Data Rekomendasi</title>
...
<div class="card-header">
    <h4 class="fw-bold mb-0" style="color:#fff"><i class="bi bi-pencil-square me-2" style="color:var(--amber)"></i>Edit Data Rekomendasi</h4>
    <div class="small-muted">Ubah data peserta — nomor surat: <?= htmlspecialchars($data['nomor_surat']) ?></div>
</div>
<div class="card-body">
<form action="update.php" method="POST">
<input type="hidden" name="id" value="<?= $data['id']; ?>">

<div class="section-title amber">Data Surat</div>
<div class="row g-3 mb-4">
<div class="col-md-8">
<label class="form-label">Nomor Surat</label>
<input type="text" name="nomor_surat" id="nomor_surat" class="form-control" value="<?= htmlspecialchars($data['nomor_surat']); ?>" placeholder="500.15.14.2/..../DTKT/IV/2026 (opsional)">
</div>
<div class="col-md-4">
<label class="form-label">Tanggal Surat</label>
<input type="date" name="tanggal_surat" id="tanggal_surat" class="form-control" value="<?= $data['tanggal_surat']; ?>">
</div>
</div>

<div class="section-title amber">Data Peserta <span style="color:rgba(255,255,255,.4);font-weight:400;text-transform:none">(Almarhum/Almarhumah)</span></div>
<div class="row g-3 mb-4">
<div class="col-md-6">
...
<input type="text" name="nik_peserta" id="nik_peserta" class="form-control nik-input" maxlength="16" inputmode="numeric" value="<?= htmlspecialchars($data['nik_peserta']); ?>" required>
<div id="feedback_nik_peserta" class="nik-feedback"></div>
</div>
<div class="col-md-3">
<label class="form-label">Pekerjaan</label>
<input type="text" name="pekerjaan_peserta" class="form-control" value="<?= htmlspecialchars($data['pekerjaan_peserta'] ?? ''); ?>">
</div>
<div class="col-12">
<label class="form-label">Alamat</label>
<textarea name="alamat_peserta" class="form-control" placeholder="Alamat lengkap peserta"><?= htmlspecialchars($data['alamat_peserta'] ?? ''); ?></textarea>
</div>
<div class="col-md-4">
<label class="form-label">Tanggal Meninggal</label>
<input type="date" name="tanggal_meninggal" class="form-control" value="<?= htmlspecialchars($data['tanggal_meninggal'] ?? ''); ?>">
</div>
</div>

<div class="section-title amber">Data Agama</div>
<div class="row g-3 mb-4">
<div class="col-md-4">
<label class="form-label">Agama</label>
<select name="agama" class="form-select">
<option value="">-- Pilih --</option>
<?php foreach(['Islam','Kristen Protestan','Kristen Katolik','Hindu','Buddha','Konghucu'] as $opt): ?>
...
<input type="text" name="denominasi" class="form-control" value="<?= htmlspecialchars($data['denominasi'] ?? ''); ?>" placeholder="Contoh: NU, Muhammadiyah, dll">
</div>
</div>

<div class="section-title amber">Data Ahli Waris</div>
<div class="row g-3 mb-4">
<div class="col-md-6">
<label class="form-label">Nama Lengkap</label>
<input type="text" name="nama_ahli_waris" class="form-control" value="<?= htmlspecialchars($data['nama_ahli_waris'] ?? ''); ?>">
</div>
<div class="col-md-3">
<label class="form-label">Hubungan Keluarga</label>
<select name="hubungan_ahli_waris" class="form-select">
<option value="">-- Pilih --</option>
<?php foreach(['Suami','Istri','Anak kandung','Orang tua','Saudara kandung','Lainnya'] as $opt): ?>
<option value="<?= $opt ?>" <?= ($data['hubungan_ahli_waris'] ?? '') === $opt ? 'selected' : '' ?>><?= $opt ?></option>
<?php endforeach; ?>
</select>
</div>
<div class="col-md-3">
<label class="form-label">NIK</label>
<input type="text" name="nik_ahli_waris" id="nik_ahli_waris" class="form-control nik-input" maxlength="16" inputmode="numeric" value="<?= htmlspecialchars($data['nik_ahli_waris'] ?? ''); ?>">
<div id="feedback_nik_ahli_waris" class="nik-feedback"></div>
</div>
<div class="col-12">
<label class="form-label">Alamat</label>
<textarea name="alamat_ahli_waris" class="form-control" placeholder="Alamat lengkap ahli waris"><?= htmlspecialchars($data['alamat_ahli_waris'] ?? ''); ?></textarea>
</div>
<div class="col-md-4">
<label class="form-label">No. HP</label>
<input type="text" name="no_hp_ahli_waris" class="form-control" value="<?= htmlspecialchars($data['no_hp_ahli_waris'] ?? ''); ?>">
</div>
</div>

<div class="section-title amber">Status Pengurusan</div>
<div class="row g-3 mb-4">
<div class="col-md-6">
<label class="form-label">Status</label>
<select name="status" class="form-select">
<option value="Belum Diurus" <?= ($data["status"] ?? "Belum Diurus") === "Belum Diurus" ? "selected" : "" ?>>Belum Diurus</option>
<option value="Sudah Diurus" <?= ($data["status"] ?? "") === "Sudah Diurus" ? "selected" : "" ?>>Sudah Diurus</option>
</select>
</div>
</div>

<div class="d-flex gap-2 pt-2 flex-wrap">
<button class="btn-glass solid px-4"><i class="bi bi-check-lg"></i> Update</button>
<a href="cetak_surat.php?id=<?= $id ?>&preview=1" class="btn-glass info px-4"><i class="bi bi-printer"></i> Cetak Surat</a>
...
function initNikValidation(){
    ...
    inputs.forEach(function(input){
        ...
        input.addEventListener('blur', function(){ updateNikFeedback(this); });
    });
